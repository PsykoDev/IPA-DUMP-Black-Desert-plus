var module686 = require('./686'),
  module687 = require('./687'),
  s = {};

for (var o in module686) module686.hasOwnProperty(o) && (s[module686[o]] = o);
...
u.get.rgb = function (n) {
  ...
  if ((s = n.match(/^#([a-f0-9]{6})([a-f0-9]{2})?$/i))) {
    ...
    if (!(s = n.match(/^rgba?\(\s*([+-]?[\d\.]+)\%\s*,\s*([+-]?[\d\.]+)\%\s*,\s*([+-]?[\d\.]+)\%\s*(?:,\s*([+-]?[\d\.]+)\s*)?\)$/)))
      return (s = n.match(/(\D+)/)) ? ('transparent' === s[1] ? [0, 0, 0, 0] : (h = module686[s[1]]) ? ((h[3] = 1), h) : null) : null;
